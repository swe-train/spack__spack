...
class Variant:
    ...
    def validate_or_raise(self, vspec, pkg_cls=None):
        ...
        if invalid_values:
            pkg_info = " in package '{pkg_cls.name}'" if pkg_cls is not None else ""
...
def prevalidate_variant_value(
    ...
    given spec, and raises if we know *before* concretization that the value cannot occur.
    ...
    if value == "*" or variant.name in spack.directives.reserved_names:
        return
    ...
    #        raise InvalidVariantValueError(
    #            f"variant '{variant.name}' does not exist in package '{pkg_cls.name}'{when}"
    #        )

    # if we get here, the variant value can't be valid because spec doesn't intersect
    # with any configuration where the variant exists
    if errors:
        raise InvalidVariantValueError(
            "multiple invalid variant values:", "".join(s.message for s in errors)
        )
